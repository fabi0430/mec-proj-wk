...
class Manipulador(Node):
    def __init__(self):
        ...
        self.past_pos = np.array([0.0, 0.0, 0.0, 0.0])
        self.speed = 0.005
    ...
    def controller_read(self):
        ...
        self.current_pos[2] += controller_input["z"] * self.speed  # Up/Down
    ...
    def publicar_posicion(self):
        if not(self.current_pos[0] == self.past_pos[0] and self.current_pos[1] == self.past_pos[1] and self.current_pos[2] == self.past_pos[2] and self.current_pos[3] == self.past_pos[3]):
            msg = Twist()
            ...
            self.past_pos[0] = self.current_pos[0]
            self.past_pos[1] = self.current_pos[1]
            self.past_pos[2] = self.current_pos[2]
            self.past_pos[3] = self.current_pos[3]
